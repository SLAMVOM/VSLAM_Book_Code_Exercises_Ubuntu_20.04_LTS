/************************************************************************************************************************
* This script performs a curve-fitting parameter estimation task using the Levenberg-Marquardt (LM) method
...
int main(int argc, char **argv) {
    ...
    int N = 5000; // number of data points used during the curve fitting
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        Eigen::Vector3d b = Eigen::Vector3d::Zero(); // bias term
        cost = 0;

        for (int i = 0; i < N; i++) {
            double xi = x_data[i], yi = y_data[i]; // the ith data point
            double error = yi - exp(ae * xi * xi + be * xi + ce); // the model function to be fitted
            Eigen::Vector3d J; // Jacobian vector, derivative of cost wrt a, b, c
            ...
            b += -inv_sigma * inv_sigma * error * J; // [3x1]
        ...
        for (int i = 0; i < N; i++) {
            double xi = x_data[i], yi = y_data[i]; // the ith data point
            double error = yi - exp(a_tmp * xi * xi + b_tmp * xi + c_tmp); // the model function to be fitted
