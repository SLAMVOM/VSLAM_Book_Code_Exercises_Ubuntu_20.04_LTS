/*
* This script performs semi-dense mono reconstruction.
* The code is modified based on dense mono-reconstruction
* in Gao's VSLAM book CH12.
*
* The function that is changed is the update() function,
* where image gradient is first computed by using the Laplacian filter.
* Then, depth is computed only when the image gradient at the pixel location
* is greatee than a certain threshold.
*/


// Author: MT
// Creation Date: 2022-May-18
// Previous Edit: 2022-May-18
...
/***********************************************************************************************
* This program illustrates a semi-dense depth estimation under a known trajectory using a mono camera
* Using epipolar line search + NCC matching for depth map construction
* *********************************************************************************************/

// --------------------------------------------------------------------------------------------
#define THRESHOLD_GRAD 50     // depth is computed only if gradient at a px is greater than threshold
...
* @param pt_curr        current point
* @param Aff_uv         affine transformation
* @return               NCC score
*/
double NCC(const Mat &ref, const Mat &curr, const Vector2d &pt_ref, const Vector2d &pt_curr, const Matrix2d &Aff_uv);
...
int main(int argc, char** argv) {
    ...
    double init_depth = 2.0;                                // initial depth value // 1.5 - Key param to tune
    double init_cov2 = 0.04;                                 // initial covariance value of inverse depth // 0.02 - Key param to tune
...
bool update(const Mat &ref, const Mat &curr, const SE3d &T_C_R, Mat &depth, Mat &depth_cov2) {
    ...
    // In the semi-dense method, we only compute the depth value at pixel locations where the
    // image gradient is greater than a certain threshold.
    // The image gradient is determined in only the reference images using the Laplacian filter.
    int kernel_size = 3;
    ...
    cv::Mat abs_grad_ref, img_grad_ref;
    cv::Laplacian( ref, img_grad_ref, ddepth, kernel_size, scale, delta, cv::BORDER_DEFAULT );
    // coverting back to CV_8U - [0, 255]
    cv::convertScaleAbs( img_grad_ref, abs_grad_ref ); // implemented as: scaling->taking absolute val->casting to uint8
    
    for (int x = boarder; x < width - boarder; x++)
        for (int y = boarder; y < height - boarder; y++) {
            
            // if the image gradient at the pixel location is smaller than the threshold, continue
            if (abs_grad_ref.at<uchar>(y, x) < THRESHOLD_GRAD)
                continue;
...
bool epipolarSearch(
    ...
    Vector3d P_ref = f_ref * depth_mu;      // p vector in the reference frame, O1P in Figure 12-4 in VSLAM book Ed.2 
    ...
    //// To prevent d_max < 0
    if (d_max < 0.2) d_max = 0.2; // 0.3 - Key param to tune
    ...
    if (d_min > 10.0) d_min = 10.0; // 2.0 - Key param to tune
    ...
    //// Calculate the values that are needed for the Affine Pre-transform
    // NOTE: the d_R and d_C in Eq.(12.12)-(12.14) should be z_R and z_C,
    // where z_R is the third element of P_ref; and 
    // z_C is the third element of (T_C_R * P_ref)
    // We need to scale the affine transformation by (z_R / z_C)
    // Note that the only part of the matrix we need is the top-left 2x2 block of the affine matrix
    Eigen::Matrix3d Aff_C_R = Eigen::Matrix3d::Identity();
    Aff_C_R = K_cam * T_C_R.so3().matrix() * K_cam.inverse(); // from reference to current frame
    Vector3d P_curr = T_C_R * P_ref; // vector connects the optical center of the current frame and the landmark point
    Aff_C_R = (P_ref(2) / P_curr(2)) * Aff_C_R; // scaling the affine transform by the estimated z values of the landmark point
    Eigen::Matrix2d Aff_uv = Aff_C_R.block<2,2>(0,0); // extracting the top-left 2x2 block from the affine matrix

    // searching along the epipolar line, using the mean depth point as center, each left and right extent for half len
    double best_ncc = -1.0;
    Vector2d best_px_curr;
    for (double l = -half_length; l <= half_length; l += 0.5){  // l += sqrt(2)=0.7
        ...
        double ncc = NCC(ref, curr, pt_ref, px_curr, Aff_uv);
        if (ncc > best_ncc) {
            best_ncc = ncc;
...
double NCC(
    const Mat &ref, const Mat &curr,
    const Vector2d &pt_ref, const Vector2d &pt_curr, const Matrix2d &Aff_uv) {
    ...
    for (int x = -ncc_window_size; x <= ncc_window_size; x++) // x for columns
        for (int y = -ncc_window_size; y <=ncc_window_size; y++) { // y for rows
            ...
            double value_curr = getBilinearInterpolatedValue(curr, pt_curr + Aff_uv * Vector2d(x, y));
...
void evaluateDepth(const Mat &depth_truth, const Mat &depth_estimate) {
    ...
    double maxi, mini;
    cv::minMaxLoc(depth_truth, &mini, &maxi); // get the maximum and minimum value
    std::cout << "Mean, max, and min in truth: " << cv::mean(depth_truth) << "   " <<  maxi << "   " << mini << endl;

    double maxi_e, mini_e;
    cv::minMaxLoc(depth_estimate, &mini_e, &maxi_e); // get the maximum and minimum value
    std::cout << "Mean, max, and min in esti.: " << cv::mean(depth_estimate) << "   " <<  maxi_e << "   " << mini_e << endl;   

    cout << "Average squared error = " << ave_depth_error_sq << ", average error: " << ave_depth_error << "\n" << endl;
}
