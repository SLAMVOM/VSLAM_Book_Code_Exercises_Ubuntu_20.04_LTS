...
// define a class for accumulating Jacobians in parallel
...
public:
    ...
    /// accumulate Jacobian for a range
    void accumulate_jacobian(const cv::Range &range);

    /// obtain the Hessian matrix, H
...
private:
    ...
    Sophus::SE3d &T21;
    VecVector2d projection; // the projected points
...
inline float GetPixelValue(const cv::Mat &img, float x, float y) {
    ...
    uchar *data = &img.data[int(y) * img.step + int(x)]; // img in cv is stored row-by-row in the memory
    float xx = x - floor(x);
    float yy = y - floor(y);
    return float(
        (1 - xx) * (1 - yy) * data[0] +
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nPoints; i++) {
        ...
        double depth = fx * baseline / disparity; // converting disparity value to depth
...
void DirectPoseEstimationSingleLayer(
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        if (std::isnan(update[0])) {
            ...
            cout << "Update is nan, breaking the updating process" << endl;
            break;
        }
    ...
    cout << "Time used by single-layer direct method: " << time_used.count() << "s." << endl;
    ...
    for (size_t i = range.start; i < range.end; i++) {
        ...
        for (int x = -half_patch_size; x <= half_patch_size; x++)
            for (int y = -half_patch_size; y <= half_patch_size; y++) {
                ...
                // Jacobian = de / dT = dI / dx = - (dI / du) * (du / dx)
...
void DirectPoseEstimationMultiLayer(
    ...
    double fxG = fx, fyG = fy, cxG = cx, cyG = cy; // store the initial values of camera settings at the original scale
    for (int level = pyramids - 1; level >= 0; level--) {
        ...
        // scale fx, fy, cx, cy by the same factor as the image in different pyramid levels
        fx = fxG * scales[level];
        fy = fyG * scales[level];
        cx = cxG * scales[level];
        cy = cyG * scales[level];
