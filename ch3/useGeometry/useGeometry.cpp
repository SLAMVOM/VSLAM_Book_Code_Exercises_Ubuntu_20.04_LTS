...
int main(int argc, char **argv) {
    ...
    // The rotation vector uses AngleAxis representation, the underlying layer is not
    // directly Matrix, but the operation can be treated as a matrix (because the 
    ...
    // Quaternion
    // You can assign AngleAxis directly to quanternions, and vice versa
    Quaterniond q = Quaterniond(rotation_vector); // convert rotation vector to unit quaternion
    cout << "quanternion from rotation vector = " << q.coeffs().transpose() << endl;
    // Note that the order of coeffs is (x, y, z, w), w is the real part, the first three are
    // the imaginary part
    // It can also assign a rotation matrix to a quaternion
    q = Quaterniond(rotation_matrix);
    cout << "quaternion from rotation matrix = " << q.coeffs().transpose() << endl;
    // Rotate a vector to a quaternion and use overloaded multiplication
