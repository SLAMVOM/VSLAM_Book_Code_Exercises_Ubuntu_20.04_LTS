...
int main(int argc, char **argv) {
    ...
    // ravel the pts_3d_eigen to make a double array for updating the landmarks
    double *pts_3d_ba;
    pts_3d_ba = new double[3 * pts_3d.size()];
    for (size_t i = 0; i < pts_3d.size(); i++) {
        pts_3d_ba[3*i] = pts_3d_eigen[i][0];
        pts_3d_ba[3*i+1] = pts_3d_eigen[i][1];
        pts_3d_ba[3*i+2] = pts_3d_eigen[i][2];
    }
    ...
    bundleAdjustmentCeres(pts_3d_eigen, pts_2d_eigen, K, pose_ceres, pts_3d_ba);
...
public:
    ...
    bool operator()(const T *const camera, const T *const point, T *residuals) const {
        ...
        // Rodrigues' formula
        AngleAxisRotatePoint(camera, point, p_cur); // passing in camera, only the first three elements are used
    ...
    static ceres::CostFunction *Create(const Eigen::Vector2d p2d, 
                                       const cv::Mat K) {
        return (new ceres::AutoDiffCostFunction<ReprojectionError, 2, 6, 3>( // 2D error; 6D pose
            new ReprojectionError(p2d, K)));
    }
...
void bundleAdjustmentCeres(
    ...
    for (int j = 0; j < points_3d.size(); j++) {
        if (j < 5){
            cout << "[" << points_3d[j].transpose() << "]\t[" << points_3d_ba[3*j] << " " << points_3d_ba[3*j+1] << " "<< points_3d_ba[3*j+2] << "]\n";
        }
        err += (points_3d[j][0]-points_3d_ba[3*j]) * (points_3d[j][0]-points_3d_ba[3*j]) +
              (points_3d[j][1]-points_3d_ba[3*j+1]) * (points_3d[j][1]-points_3d_ba[3*j+1]) + 
              (points_3d[j][2]-points_3d_ba[3*j+2]) * (points_3d[j][2]-points_3d_ba[3*j+2]);
    }
