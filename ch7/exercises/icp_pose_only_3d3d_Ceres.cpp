...
int main(int argc, char **argv) {
    ...
    bundleAdjustmentCeres(pts1_3d_eigen, pts2_3d_eigen, pose_ceres);
    ...
    for (int i = 0; i < 5; i++) { // show the first three points
        ...
        cout << "p2 = " << pts2[i] << endl;
        cout << "(R * p2 + t) =\n" << 
            // R * (Mat_<double>(3,1) << pts2[i].x, pts2[i].y, pts2[i].z) + t // using this or below line
...
public:
    EuclideanError(Eigen::Vector3d pt1, Eigen::Vector3d pt2) : _pt1(pt1), _pt2(pt2) {}
    ...
    bool operator()(const T *const camera, T *residuals) const {
        ...
        T p_ref[3]; // point coordinates under the reference frame
        p_ref[0] = T(_pt1[0]); // x_ref
        p_ref[1] = T(_pt1[1]); // y_ref
        p_ref[2] = T(_pt1[2]); // z_ref
        // camera[0,1,2] are the angle-axis rotation
        // Rodrigues' formula - defined in "rotation.h"
        AngleAxisRotatePoint(camera, p_ref, predictions); // passing in camera, only the first three elements are used
    ...
    static ceres::CostFunction *Create(const Eigen::Vector3d pt1,
                                       const Eigen::Vector3d pt2) {
        return (new ceres::AutoDiffCostFunction<EuclideanError, 3, 6>( // 3D error; 6D pose
            new EuclideanError(pt1, pt2)));
    }
...
void bundleAdjustmentCeres(
    ...
    for (size_t i = 0; i < pts1.size(); i++) {
        ...
        cost_function = EuclideanError::Create(pts1[i], pts2[i]);
    ...
    options.linear_solver_type = ceres::DENSE_SCHUR; // solver types: SPARSE_SCHUR, DENSE_SCHUR, DENSE_QR, DENSE_NORMAL_CHOLESKY, SPARSE_NORMAL_CHOLESKY
