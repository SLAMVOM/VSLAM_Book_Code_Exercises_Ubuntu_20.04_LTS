...
* including landmark optimization by explictly implementing using LM.
*************************************************************************/
...
int main(int argc, char **argv) {
    ...
    const int OPTIMIZE_METHOD = 1; // 0 for Gauss-Newton; Levenberg-Marquardt, otherwise
    cout << "Calling bundle adjustment by either Levenberg-Marquardt or Gauss-Newton" << endl;
    // Sophus::SE3d pose_gn;
    ...
    bundleAdjustmentLM(pts_3d_eigen, pts_2d_eigen, K, pose_gn, true, pts_3d_ba, OPTIMIZE_METHOD); // false if not optimizing landmarks
...
void bundleAdjustmentLM(
    ...
    // Levenberg_Marquardt parameters
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> D; // the diagonal matrix in L-M
    double L_upper = 11.0, L_lower = 9.0; // hyperparameters for update mode 1 - "Levenberg"
    double lambda_init = 1e-2, epsilon = 1e-1; // initial value of damping param, and update acceptance threshold
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int i = 0; i < points_3d.size(); i++) {
            ...
            // concatenate the Jacobian of e wrt the current landmark into the J_all matrix
        ...
        J_all.setZero();
        for (int pt_idx = 0; pt_idx < vec_jac_pts.size(); pt_idx++) {
            J_all.block<2, 3>(pt_idx*2, pt_idx*3) = vec_jac_pts[pt_idx];
        }

        // first compute the H_all
        H_all = J_all.transpose() * J_all; // [(6+3n) x (6+3n)]

        // L-M steps
        if (LM_method != 0) { // "Levenberg" update method
            D = H_all.diagonal().asDiagonal(); // construct the diagonal matrix D
            H_all += lambda * D; // [(6+3n) x (6+3n)]
        }
        ...
        if (LM_method != 0) { // using Levenberg-Marquardt if true
            // L-M update of lambda
            cost_tmp = 0.0; // reset the temporary cost
            for (int i = 0; i < points_3d.size(); i++) {
                Eigen::Vector3d dx_tmp(dx[3*i], dx[3*i+1], dx[3*i+2]);
                // compute the current p^{i}_{c2}
                Eigen::Vector3d pc = pose * (points_3d_ba[i] + dx_tmp); // [3 x 1]
                double inv_z = 1.0 / pc[2]; // the third element as normalizing scalar
                double inv_z2 = inv_z * inv_z;
                Eigen::Vector2d proj(fx * pc[0] / pc[2] + cx, fy * pc[1] / pc[2] + cy); // projected pixel coor. on frame 2
                ...
                cost_tmp += e.squaredNorm(); // the overall cost is the sum of reprojection errors of all landmarks
            }
            ...
            if (rho > epsilon) {
                ...
                for (int j = 0; j < points_3d.size(); j++) {
                    points_3d_ba[j][0] += dx[3*j];
                    points_3d_ba[j][1] += dx[3*j+1];
                    points_3d_ba[j][2] += dx[3*j+2];
                }
                lambda = std::max(lambda / L_lower, 1e-7);
            } else {
                lambda = std::min(lambda * L_upper, 1e7);
            }
        } else { // if not using L-M, then use Gauss-Newton
            for (int j = 0; j < points_3d.size(); j++) {
                points_3d_ba[j][0] += dx[3*j];
                points_3d_ba[j][1] += dx[3*j+1];
                points_3d_ba[j][2] += dx[3*j+2];
