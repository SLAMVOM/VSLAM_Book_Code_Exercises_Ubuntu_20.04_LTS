...
void G2OBAFunc( // function reference: cnblogs.com/newneul/p/8545450.html
    ...
    for (int i = 0; i < points1_3d.size(); i++) {
        auto point = new g2o::VertexSBAPointXYZ(); // 3d landmark point class by g2o
    ...
    // Adding camera parameters to be optimized (the last element is 0, assume fx=fy, then optimize pose)
    ...
    for(int i = 0; i < points1_2d.size(); i++) {
        auto edge = new g2o::EegeProjectXYZ2UV; // set the edge that connects to the first frame
        // binary edge
