...
void bundleAdjustmentGaussNewton(
    ...
    for (int iter = 0; iter < iterations; iter++) {
        Eigen::Matrix<double, Eigen::Dynamic, 2> J_all; // the jacobian matrix is of [(6+3n) x 2]
        ...
        for (int i = 0; i < points_3d.size(); i++) {
            ...
            if (i == 0) {
                J_all = J.transpose(); // [6 x 2]
                b_all = b; // [6 x 1]
            }
            // concatenate the Jacobian of e wrt the current landmark into the J_all matrix
            J_all.conservativeResize(J_all.rows()+3, J_all.cols());
            J_all.block<3, 2>(J_all.rows()-3, 0) = J_pts.transpose();
        ...
        H_all = J_all * J_all.transpose(); // [(6+3n) x (6+3n)]
        ...
        for (int j = 0; j < points_3d.size(); j++) {
            ...
            points_3d_ba[j][1] += dx[6+3*j+1];
            points_3d_ba[j][2] += dx[6+3*j+2];
        }
